 ...
 * */
export const useTable = (apiUrl: (params: any) => Promise<any>, initParam: any = {}, isPageable = true, listKey = 'list') => {
  ...
  const getTableList = async() => {
    try {
      // 更新查询参数
      updatedTotalParam()
      Object.assign(state.totalParam, {param: initParam})
      const data = await apiUrl(state.totalParam)
      data[listKey] && (state.tableData = data[listKey])
      // 解构后台返回的分页数据(如果有分页更新分页信息)
      const { count } = data
      isPageable && count && updatePageable({ page: pageParam.value.page, pageSize: pageParam.value.pageSize, total: count })
    } catch (error) {
      console.log(error)
    }
  }
  ...
  const updatedTotalParam = () => {
    ...
    Object.assign(state.totalParam, {param: nowSearchParam}, isPageable ? pageParam.value : {})
  }
